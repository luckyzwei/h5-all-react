...
export const saveCookie = (name, data, time) => {
    ...
    if (time) {
        ...
        let newData = window.btoa ? window.btoa(escape(data)) : escape(data)
        document.cookie = name + "=" + newData + ";expires=" + exp.toGMTString() + ';path=/';
    }
    ...
    for (var i = 0; i < aCookie.length; i++) {
        ...
        if (name === aCrumb[0])
            if(time){
                ...
                return  window.atob ? unescape(window.atob(aCrumb[1])) : unescape(aCrumb[1])
            }
        // return unescape(aCrumb[1])
    }
    return null;
