...
from ut.base import redis
from config import settings
from app.route_requests import post_json, do_get
from ut.response import response_json
from ut.constants import JS_API_TICKET_REDIS_KEY, WECHAT_AUTH_ERROR, GPET_WORLD_ACCESS_TOKEN_REDIS_KEY, \
    OPEN_ACCESSTOKEN_REDIS_KEY, BASIC_PATH
...
async def before_server_start(_app, _loop):
    _app.redis = redis.init_app(_loop, settings['REDIS'])
...
async def app_status(request):
    '''校验当前服务状态'''
    data = {'status': 2, 'upGradeEndTime': f'{now.month}月{now.day}日{now.hour+3}时'}
    return response_json(data)


@app.route('/wechat/user_info', methods=['GET'])
...
@app.route('/wechat/js/ticket', methods=['GET'])
...
async def get_user_from_wechat(access_token, open_id):
    ...
    # 此处是为了把信息全部读出来
    resp_json = ujson.loads(await resp_json.content.read(100000))
...
app.static('/static', os.path.join(BASIC_PATH, 'static/grouppet/build/static'))
app.static('/images', os.path.join(BASIC_PATH, 'static/grouppet/build/images'))
app.static('/css', os.path.join(BASIC_PATH, 'static/grouppet/build/css'))


@app.route('/', methods=['POST', 'GET', 'DELETE', 'PUT'])
@app.route('/<path:path>', methods=['POST', 'GET', 'DELETE', 'PUT'])
async def catch_all_views(request, path=''):
    file_path = os.path.join(BASIC_PATH, 'static/grouppet/build/index.html')
    try:
